...
{//alert(data.msg.error.length);alert(data.msg.success.length);
  ...
  else if(data.msg.success.length > 0)
  {
	  $(".error").hide();
	  $(".err"+data.msg.successbox).show();
	  $(".err"+data.msg.successbox).html(data.msg.success);
	  
	   /*if ((obj = $('#imageup')).length &&  data.image.path) {
		...
		if(data.image.path.length>0){
			
			//document.getElementById('you').innerHTML='<img src="'+ data.image.path +'" alt="" class="update_img"/><a href="javascript://" onClick="remove_image()">Remove</a>'; //'+ data.update.id +'
			
			$("#update_image").attr({src:  data.image.path});
			$("#update_id").val(data.update.id);
		...
		if(data.msg.image_media=='update')
		{
		
		//$(".error").hide();
		$('#close_fancy').trigger('click');
		$('#update_suc').trigger('click');
		...
		$("#file1").val('');
		$("#update_video1").val('');
		$('#update_text1').attr({placeholder: "Get more donations by posting frequent update messages to your page!"});
		...
		if(data.msg.video_media!='showvideobox' && data.msg.video_media!='')
		{
			...
			//$("#media_error_no"+data.msg.video_media).show();
			// $("#media_error_no"+data.msg.video_media).html(data.msg.success);
			document.getElementById('ajaxdata').innerHTML=data.msg.returnd;
			/*append_image(data.msg.media_image_id,document.getElementById('imagec').value);
