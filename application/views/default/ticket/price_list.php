...
?>
				                <table class="table" id="ticket_table">
	                                ...
	                                ?>
		                                <tr>
		                                  	 ...
		                                  	 <?php
						                        ...
						                        ?>
		                                  	 <?php echo set_event_currency(0,$event_details['id']);//set_currency(0);?>x
		                                  	 <?php echo $ticket_qty[$free['id']];?>
		                                  	 </span>
		                                  	 </td>
		                                  	<!--  <td><?php // echo set_event_currency(0,$event_details['id']);//set_currency(0);?></td> -->
		                                  <!-- 	 <td><?php // echo $ticket_qty[$free['id']];?></td> -->
		                                  	 <td>
		                                  	 <?php
		                                  	 if($attendee=='attendee' && isset($ticket_amount)){ $paid['buyer_total'] = $ticket_amount[$free['id']]; $amt = $paid['buyer_total'];}
		                                  	  if($amt==''){	$amt = 0;}
			                                  	 
			                                  	  echo set_event_currency($amt,$event_details['id']);//set_currency($amt); 
			                                  	  $total+=$amt;
			                                  	  $ticket_amt = $ticket_amt_per;
		                                  	 ?>
		                                  	 <?php //echo  set_event_currency(0,$event_details['id']);//set_currency(0); $total+=0; ?>
					                ...
					                <?php /*paid ticket details start*/
                                                           // print_r($ticket_qty);
                                                           // print_r($paid_tickets);
                                                        ?>
					                <?php 
	                                ...
	                                ?>
	                                	...
	                                	<br/>	<span class="value-info">
		                                  	 (<?php 
		                                  	 	...
		                                  	 	echo set_event_currency($paid['price'],$event_details['id']);//set_currency($paid['price']);
		                                  	 ?> 
		                                  	+
		                                  	 <?php 				
		                                   		if($paid['fee']>0){
													...
													$payment_gateway_flat_fee =  $currency['payment_gateway_flat_fee'];
							                      	...
							                      	//$paid['fee']=$paid['fee']+$currency_amt;	    	
		                                   			...
		                                   			if($attendee=='attendee'){	  
							                        	...
							                        	$ticket_amt_per = 0;
							                        	$add_geteway_fee = 0;                       		
														//$ticket_amt_per = $ticket_amount[$paid['id']];
						                        	}else{
						                        		$gateway_fee = 0;
						                        ...
						                        if(($event_details['event_pass_fees']==1) || ($event_details['event_pass_fees']==3 && $paid['service_fee']==1)) {
						                        	...
						                        	// $paidFee = $paid['fee'] + $add_geteway_fee;
						                        	if($promo_code_id > 0){
					                                  	...
					                                  	if($promo_code){
				                                  	 		if(in_array($paid['id'], $promo_tkts_arr)){	
				                                  	 			if($promo_code['disc_perc'] != '' && $promo_code['disc_perc'] == 100){
																	...
																	/*$perc = (($paid['price']  * $promo_code['disc_perc']) / 100);
														       		$paid['price'] = $paid['price'] - $perc;
														       		$fee = (($paid['price']  * $site_setting['paid_ticket_fee']) / 100);*/
				                                  	 				echo set_event_currency($paidFee,$event_details['id']);//set_currency($fee);
				                                  	 			}
				                                  	 		} else {
				                                  	 			echo set_event_currency($paidFee,$event_details['id']);//set_currency($fee);
				                                  	 		}
		                                  	 ...
		                                  	 <!-- <td><?php // echo $ticket_qty[$paid['id']];?></td> -->
		                                  	 <td>
		                                  	 <?php 
		                                  	 	//$ticket_amt_per='';
		                                  	 	if($attendee=='attendee'){
		                                  	 		//$paid['buyer_total'] = $paid['buyer_total'] - $paid['fee'];
			                                  	 ...
			                                  	 if($event_details['event_pass_fees']==1){ 
			                                  	 	//$amt = $ticket_qty[$paid['id']] * ($paid['price'] + $fee);
			                                  	 	if($attendee!="attendee"){
		                                  	 ...
		                                  	 ?>
		                                  	 	...
		                                  	 	// } elseif($event_details['is_wallet'] == 1)  {
		                                  	 	// 	$wallet_split_gateway_fee = 0;
		                                  	 	} else {
	                                ...
	                                ?>
		                                <tr>
		                                  	...
		                                  	<br/> <span class="value-info"><?php echo set_event_currency($ticket_qty[$donation['id']],$event_details['id']);//set_currency($ticket_qty[$donation['id']]);?>
											  + <?php 
		                                  	 if($attendee=="attendee"){
                                  	 			$ticket_type = "1";
		                                  	 	$fee = 0;
		                                  	 	...
		                                  	 	// $ticket_amt += $ticket_qty[$donation['id']];
	                                  	 		// $total_fees += $fee;
	                                  	 		// $donation_gateway_fee = ($ticket_qty[$donation['id']] + $fee + $currency['payment_gateway_flat_fee']) / (1 - ($currency['payment_gateway_fee'] /100));
	                                  	 		// $donation_gateway_fee = $donation_gateway_fee - $ticket_qty[$donation['id']] + $fee;
	                                  	 		// $donation_gateway_fee = number_format($donation_gateway_fee, 2);
	                                  	 		// $donation_gateway_fee= (float)$donation_gateway_fee;

	                                  	 		// $ticket_amt_per = $ticket_qty[$donation['id']];
												...
												$total_fees += $fee;
	                                  	 		if($event_details['event_pass_fees']==1 || ($event_details['event_pass_fees']==3 && $donation['service_fee']==1)){
													...
													// $ticket_amt = $ticket_qty[$donation['id']] - $donation_gateway_fee - $paid_Fee;
	                                  	 			$ticket_type = "3";
											...
											if(($event_details['event_pass_fees']==1) || ($event_details['event_pass_fees']==3 && $donation['service_fee']==1)){
												echo set_event_currency($donation_ticket_gateway_fee,$event_details['id']);//set_currency($fee);
											} else {
												echo set_event_currency(0,$event_details['id']);//set_currency(0)	;
											}
		                                  	 ?></span>
		                                  	 ...
		                                  	 <?php 
												 // if($event_details['event_pass_fees']==1){
											 	// 	$ticket_type = "2";
												 // 	$amt = $ticket_qty[$donation['id']] + $fee + $donation_gateway_fee;
												 // } else if($event_details['event_pass_fees']==3){
												 // 	if($donation['service_fee']==1){
											 	// 		$ticket_type = "2";
												 // 		$amt = $ticket_qty[$donation['id']] + $fee + $donation_gateway_fee;
												 // 	}else {
											 	// 		$ticket_type = "1";
												 ...
												 // 	$amt = $ticket_qty[$donation['id']];
													...
													// // $ticket_amt_per -= $fee;
												 // }
												   echo set_event_currency($ticket_amt_per,$event_details['id']);//set_currency($amt); 
												   $total+=$ticket_amt;
												//    $ticket_amt = number_format($ticket_amt, 2);
		                                  	 ...
		                                  	 <?php
											    if($amt){
		                                  	 	...
		                                  	 	$ticket_amt_per = $ticket_amt_per;
		                                  	 	$donation_gateway_fee = $donation_gateway_fee;
		                                  	 ?>
					                ...
					                <tr>
					                <th colspan="4" style="text-align:right;"><?php echo Total_Amount;?> <?php echo set_event_currency($total,$event_details['id']);//set_currency($total);?></th>
